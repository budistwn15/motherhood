                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <h3 class="card-title pt-2">Silahkan synchronize form gejala penyakit di bawah ini</h3>
                            </div>
                        </div>
                        <div class="card-body">
                            <form action="{{route('relasi-gejala.update',['gejala' => $gejala->id])}}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2bs4 @error('gejala_id') is-invalid @enderror" style="width: 100%;" name="gejala_id">
                                        @foreach($gejalas as $item)
                                            <option value="{{$item->id}}" @selected($item->id == $gejala->id)>{{$item->kode_gejala}} - {{$item->nama_gejala}}</option>
                                        @endforeach
                                    </select>
                                    @error('gejala_id')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2bs4 @error('penyakit_id') is-invalid @enderror" style="width: 100%;" name="penyakit_id">
                                        @foreach($penyakits as $penyakit)
                                            <option value="{{$penyakit->id}}" @selected($gejala->penyakits()->find($penyakit->id))>{{$penyakit->kode_penyakit}} - {{$penyakit->nama_penyakit}}</option>
                                        @endforeach
                                    </select>
                                    @error('penyakit_id')
                                    <div class="invalid-feedback">{{$message}}</div>
